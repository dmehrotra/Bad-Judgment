...
function secondsToString(ms) {
    var seconds = ms / 1e3;
    var numyears = Math.floor(seconds / 31536e3);
    var numdays = Math.floor(seconds % 31536e3 / 86400);
    var numhours = Math.floor(seconds % 31536e3 % 86400 / 3600);
    var numminutes = Math.floor(seconds % 31536e3 % 86400 % 3600 / 60);
    var numseconds = seconds % 31536e3 % 86400 % 3600 % 60;
    return {
        days: numdays,
        hours: numhours,
        minutes: numminutes
...
Sentence.prototype.cantSleep = function() {
    ...
    if (this.data.data.home) {
        var ar = [ "It's late, you should be asleep but you check me one more time just to be sure. The date is " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you are at home in " + this.data.data.city + ".", "You had that dream again and now you are awake. It's " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you are checking me to forget.", "You are at home and cant sleep. The date is " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you are looking for picture of yourself when you were young, again." ];
        this.advertisement = "sleep aid";
        return ar[Math.floor(Math.random() * ar.length)];
    } else {
        var ar = [ "You were having a great time with your friends in " + this.data.data.city + " on  " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " but they all went home. Now you are at the bar alone pretending to be reading a work email while actually checking me for no particular reason.", "You are in " + this.data.data.city + " and it's  " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ". You don't know how you got here and you wonder why you are on facebook. Tell me what's on your mind.", "It's  " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ". You had that dream again and when you woke up your shoes were on. You had logged in to facebook from " + this.data.data.city + ", not the same place you fell asleep." ];
...
Sentence.prototype.frequentChecker = function() {
    var ar = [ "You checked me " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago. A lot has changed, and I can't wait for you to check me again.", "You are still in " + this.data.data.city + ". As you closed your facebook window " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago you thought you saw a photo of yourself. You log back in and can't find the picture.", "When you last checked me, " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago, you noticed that someone you used to know from high school passed away.  You log back in to find out how." ];
    this.advertisement = "frequent checker";
    return ar[Math.floor(Math.random() * ar.length)];
};

Sentence.prototype.work = function() {
    var ar = [ "You are bored at work on " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ". You resize a new browser window so that it is small enough not to be seen by those around you. As you start typing 'faceb', you think to yourself, 'tomorrow I am going to look for a new job'.", "On " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " you look your boss up on facebook.  In her profile picture she is holding a cocktail, but you could have sworn she said she was sober.", "On your work computer " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ", you accidently return to facebook after typing f into google chrome.", "You are at work and you want to quit. It's " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ", you march over to your boss and ask if she want's to chat over a drink after work.  She says she is sober.  You check me for updates and find out that the identical twin of someone you used to know from high school has passed away." ];
    this.advertisement = "linkedin";
    return ar[Math.floor(Math.random() * ar.length)];
};

Sentence.prototype.workLate = function() {
    var ar = [ "You checked me " + this.data.time_since.hours + "ago. It's " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you are literally watching the hands of the clock.  Your eyes are starting to blur and it seems like the hour and minute hands are crushing time between their pincers. You think to yourself 'what am I even doing'.  You log on to facebook to find out.", "You at work late on " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ". You are hungry and tired. Your boss just doesn't pay enough attention to all the work that you are putting in and you can't help but feel like your life isn't going anywhere. As you start typing 'faceb', you think to yourself, 'tomorrow I am going to look for a new job'.", "You are at work late again on " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ". You hear somone shuffling around in the hallway.  When you peer over your partition to see who it is, you find your boss sleeping on the floor. You log into facebook and consider removing every photo of yourself" ];
    this.advertisement = "linkedin";
    return ar[Math.floor(Math.random() * ar.length)];
};
...
Sentence.prototype.wakeup = function(place) {
    if (place) {
        var ar = [ " On " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ", you wake up with the taste of cedar in your mouth and for some reason you can't remember how you managed to fall asleep with your hand in the drain of the kitchen sink.  You log in to facebook to discover that you have no new notifications.", " You wake up on " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " having had that dream again.  Except this time your boss was watching.  You log on to facebook and discover that your sister is a lesbian.", " You wake up on " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " to the smell of burning toast. At first you think you are having a heart attack, but then you realize that only happens if you smell cedar. You log on to facebook and discover that you forgot to send that message afterall." ];
        return ar[Math.floor(Math.random() * ar.length)];
    } else {
...
Sentence.prototype.checkingElsewhere = function() {
    var ar = [ "The stranger you made eye contact with on " + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " just friended you on facebook.  You log in and find out that it was an old friend from highschool.", "You discretely log on to facebook after you meet someone who shares the same name as the friend of yours who passed away - or was it his twin who died? It's" + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you should be writing`	` that email your boss want's sent by EOD.", "You are in " + this.data.data.city + ", you last logged in from here, " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago. You've come here " + this.data.data.frequency + " times this year to reflect. But you had that dream again last night, and you are wondering if you should tell someone. You log in. Why don't you tell me whats on your mind.", "You are in " + this.data.data.city + ", you last logged in from here, " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago. You check your facebook because you can't remember if your boss was drunk at the holiday party or not.  It turns out that you can't tell from the photos." ];
    return ar[Math.floor(Math.random() * ar.length)];
};
...
Sentence.prototype.earlyNotMoved = function() {
    var ar = [ "You are still in bed in " + this.data.data.previous_session_city + ". It's been" + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes, since I last heard from you. What's on your mind?", this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + ", you checked me " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago.", "After you checked me " + this.data.time_since.hours + " hours and " + this.data.time_since.minutes + " minutes ago, you fell asleep while reaching out to an old friend named Molly via Messanger.  You log in to me and discover that you don't have a friend named Molly." ];
    return ar[Math.floor(Math.random() * ar.length)];
};

Sentence.prototype.defaultSentence = function() {
    var ar = [ "The stranger you made eye contact with on " + this.data.data.date.replace("EDTIP ESTIP", "") + " just friended you on facebook.  You log in and find out that your cousin is in vacation in Mexico City.", "You discretely log on to facebook after you meet someone who shares the same name as that friend of yours who passed away - or was it his twin. It's" + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you should be sending that email your boss want's sent by EOD.", this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " in " + this.data.data.city + " you see your boss leaving a bar.  Didn't she say she was sober? You ask a coworker on a date through Messanger.", "This was the second time on " + this.data.data.date.replace("EDTIP ESTIP", "") + " that you mistook a friend for his twin on facebook.  You would like to keep it that way.", "It's" + this.data.data.date.replace("EDTIP", "").replace("ESTIP", "") + " and you are trying to figure out what college he graduated from." ];
    return ar[Math.floor(Math.random() * ar.length)];
};
