...
Sentence.prototype.hungover = function(){
	...
	str = str + " You drank so much last night in "+this.data.data.city + ' that you have to look at a wall to keep the room from spinning. You feel like did something stupid, but you cant remember what.'
...
Sentence.prototype.cantSleep = function(){
	...
	if(this.data.data.home){
		var ar = [option1, option2];
		option1 = option1 + "It's late, you should be asleep but you check me one more time just to be sure. The date is" + this.data.data.date.replace('EDTIP', '') + '.';
		option2 = option2 + "You had that dream again and now you are awake. It's " + this.data.data.date.replace('EDTIP', '') + ' and now you are checking me because my light comforts you.';
...
Sentence.prototype.wakeup = function(place){
	var str = '';
	if (place){
		str = str + " On " + this.data.data.date.replace('EDTIP', '')+ ", you wake up. You roll over onto the cool side of the pillow and check me for updates.";
